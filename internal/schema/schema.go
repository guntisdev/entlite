...
func (e Entity) ContainsUniqueKey(columns []string) ([]string, bool) {
	...
	if len(keyColumns) > 0 && holdsColumns(columns, keyColumns) {
		return keyColumns, true
	}

	for _, column := range columns {
		if field, ok := e.GetFieldByName(column); ok && field.Unique {
			return []string{field.Name}, true
		}
	}

	for _, idx := range e.Indexes {
		if idx.Unique && holdsColumns(columns, idx.FieldNames()) {
			return idx.FieldNames(), true
		}
	}
...
func (e Entity) HasAutoGeneratedKey() bool {
	...
	return idField.DefaultFunc == nil && idField.DefaultValue == nil
}
...
func (e Entity) InsertReturnsID() bool {
	return e.HasIdField() && !e.CallerSuppliesID()
}

func holdsColumns(columns, wanted []string) bool {
	for _, want := range wanted {
		found := false
		for _, column := range columns {
			if strings.EqualFold(want, column) {
				found = true
				break
			}
		}
