...
func zeroValueForFieldType(t schema.FieldType) string {
	switch t {
...
func generateCreateQuery(funcDecl *ast.FuncDecl, entity schema.Entity, inputPkg string, sqlDialect schema.SQLDialect) string {
	...
	// no id, or the caller sent it, so return only an error
	...
	if sqlDialect == schema.MySQL && !entity.HasAutoGeneratedKey() {
		exportedID := toExportedName(idField.Name)
		zeroID := zeroValueForFieldType(idField.Type)
		ignoresConflict := false
		if query, ok := entity.QueryByType(schema.QueryCreate); ok {
			ignoresConflict = query.Upsert && query.UpsertIgnore
		}
		...
		if ignoresConflict {
			// ON DUPLICATE KEY UPDATE slug = slug leaves an existing row untouched,
			// which RowsAffected reports as 0; that is this dialect's sql.ErrNoRows
			sb.WriteString(fmt.Sprintf("\tresult, err := (*%s.Queries)(q).%s(ctx, internalArg)\n", inputPkg, funcDecl.Name.Name))
			sb.WriteString(fmt.Sprintf("\tif err != nil {\n\t\treturn %s, err\n\t}\n", zeroID))
			sb.WriteString("\trows, err := result.RowsAffected()\n")
			sb.WriteString(fmt.Sprintf("\tif err != nil {\n\t\treturn %s, err\n\t}\n", zeroID))
			sb.WriteString(fmt.Sprintf("\tif rows == 0 {\n\t\treturn %s, sql.ErrNoRows\n\t}\n", zeroID))
		} else {
			sb.WriteString(fmt.Sprintf("\tif err := (*%s.Queries)(q).%s(ctx, internalArg); err != nil {\n", inputPkg, funcDecl.Name.Name))
			sb.WriteString(fmt.Sprintf("\t\treturn %s, err\n\t}\n", zeroID))
		}

		sb.WriteString(fmt.Sprintf("\treturn internalArg.%s, nil\n", exportedID))
		sb.WriteString("}\n\n")
...
func generateCreateBulkQuery(funcDecl *ast.FuncDecl, entity schema.Entity, inputPkg string, sqlDialect schema.SQLDialect) string {
	...
	paramsType := naming.ParamsName(queryName)
	internalParamsType := fmt.Sprintf("%s.%s", inputPkg, naming.ParamsName(queryName))
	rowsFunc := toUnexportedName(queryName) + "Rows"

	// no id, or the caller sent it, so there is nothing to collect
	hasID := entity.InsertReturnsID()
	idType := ""
	resultsType := "error"
	nilResult := ""
	if hasID {
