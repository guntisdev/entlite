...
SELECT *, COUNT(*) OVER() AS total_count FROM "article" WHERE author = @author AND is_featured = @is_featured AND published_at BETWEEN @min_published_at AND @max_published_at AND title LIKE @title ORDER BY published_at LIMIT sqlc.arg('limit') OFFSET sqlc.arg('offset');
