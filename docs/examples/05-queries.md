...
generated one — that is what [02-custom](../../examples/02-custom) is about.
...
message BuildTotalsRequest {
}

message BuildTotalsResponse {
  int64 sum_duration_ms = 1;
  double avg_duration_ms = 2;
  int64 max_failed_tests = 3;
}
message BranchDurationsRequest {
}
...
message BranchDurationsResponse {
  repeated BranchDurationsRow rows = 1;
}
message ListEnvBranchDurationsRequest {
  string env = 3 [(buf.validate.field).required = true];
}

message ListEnvBranchDurationsRow {
  string branch = 1;
  int64 sum_duration_ms = 2;
  double avg_duration_ms = 3;
}

message ListEnvBranchDurationsResponse {
  repeated ListEnvBranchDurationsRow rows = 1;
}
...
service BuildService {
  ...
  // without a GroupBy the aggregates fold the whole table into one row
  rpc BuildTotals(BuildTotalsRequest) returns (BuildTotalsResponse);
  // GroupBy returns one row per branch, an aggregate query names itself
  rpc BranchDurations(BranchDurationsRequest) returns (BranchDurationsResponse);
  // the groups take filters, sorting and paging like any other list
  rpc ListEnvBranchDurations(ListEnvBranchDurationsRequest) returns (ListEnvBranchDurationsResponse);
}
```
