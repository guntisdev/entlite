...
import (
	...
	"github.com/guntisdev/entlite/examples/03-optional/mysql/ent/gen/pb"
	"github.com/guntisdev/entlite/examples/03-optional/mysql/server"
)
...
func main() {
	...
	articlePath, articleHandler := pb.NewArticleServiceHandler(
		server.NewArticleServiceServer(database),
		connect.WithInterceptors(
			validate.NewInterceptor(),
			pb.NewValidateInterceptor(),
		),
	)
	mux.Handle(articlePath, articleHandler)
...
// waitForDatabase polls until mysql finishes its startup, which takes a few seconds
