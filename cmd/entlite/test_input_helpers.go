...
func (User) Fields() []entlite.Field {
	return []entlite.Field{
		...
		field.Time("created_at").Contracts(entlite.SQLC(), entlite.PROTO().ReadOnly()).DefaultFunc(time.Now).ProtoField(6).Immutable(),
		field.Time("updated_at").Contracts(entlite.SQLC(), entlite.PROTO().ReadOnly()).DefaultFunc(time.Now).ProtoField(7),
	}
}
...
func (User) Queries() []entlite.Query {
	return []entlite.Query{
		query.DefaultCRUD(),
		query.ListBy("name", "age").Limit().Offset(),
	}
}`
